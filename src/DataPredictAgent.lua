...
{agent_name}: Yes I do! You are in front of me!
]]
...
function AqwamAgentLibrary:addServerDictionary(serverName, serverDictionary)
	...
	local serverDictionary = serverDictionary or {}
	...
	serverDictionary.address = serverDictionary.address or "http://127.0.0.1:5000/v1/chat/completions"
...
function AqwamAgentLibrary:getServerDictionary(serverName)
	...
	local ServerDictionary =  self.dictionaryOfServerDictionary[serverName]

	if (not ServerDictionary) then error("The server does not exist.") end
...
function AqwamAgentLibrary:getAgentDictionary(agentName)
	...
	local AgentDictionary =  self.dictionaryOfAgentDictionary[agentName]

	if (not AgentDictionary) then error("The agent does not exist.") end
...
function AqwamAgentLibrary:getInteractorDictionary(interactorName)
	...
	local InteractorDictionary = self.dictionaryOfInteractorDictionary[interactorName]

	if (not InteractorDictionary) then error("The interactor does not exist.") end

	return InteractorDictionary
...
function AqwamAgentLibrary:addAgentActionArray(agentActionName, agentActionArray)
	...
	dictionaryOfAgentActionArray["startReinforcement"] = {"startReinforcement"}
	
	dictionaryOfAgentActionArray["stopReinforcement"] = {"stopReinforcement"}
...
function AqwamAgentLibrary:getAgentActionArray(agentActionName)
	...
	local AgentActionArray = self.dictionaryOfAgentActionArray[agentActionName]

	if (not AgentActionArray) then error("The agent action array does not exist.") end

	return AgentActionArray
...
function AqwamAgentLibrary:sendServerRequest(serverName, inputMessage, parameterDictionary)
	...
	for parameter, value in parameterDictionary do requestDictionary[parameter] = value end
	...
	local choicesArray = responseDictionary.choices
	
	local numberOfChoices = #choicesArray
	
	local randomChoiceIndex = math.random(1, numberOfChoices)

	local outputMessage = choicesArray[randomChoiceIndex].message.content
...
function AqwamAgentLibrary:bindFreeWillToAgent(agentName, functionToRun)
	...
	local initialPromptToAdd = "Currently, you are muted, but you can use \"{action_to_do}chat\" to talk, but you can use muted to store your internal thoughts without revealing them to anybody. This is a random number for random response generation. Here is the number, but ignore it: "
	...
	local reinforcementLearningEndingPrompt = "\n\nEnsure that your actions are consistent with your personality. Also, this is currently your free will response, so you are not directly chatting with anyone else and is considered an internal thought unless you choose to unmute yourself it."
		...
		while (dictionaryOfAgentDictionary[agentName]) do
			...
			if (#agentActionToDoArray == 0) then
				...
				if (model) then
					...
					local classesList = model.Model:getClassesList()
